		...
		if (UseMetaFields = 1) then
			...
			for fieldnum = 0 to NumMetaFields-1
				...
				if (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_NUMERIC) then
					...
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<input type=""text"" id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ size=""20"" value=""" & meta_query(fieldnum) & """ class=""zoom_metaform_numeric"" />" & VbCrLf
				...
				elseif (metafields(fieldnum)(METAFIELD_TYPE) = METAFIELD_TYPE_MONEY) then
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>"
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & MetaMoneyCurrency & "<input type=""text"" id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ size=""7"" value=""" & meta_query(fieldnum) & """ class=""zoom_metaform_money"" />" & VbCrLf
				else
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<label for=""" & labelID & """>" & metafields(fieldnum)(METAFIELD_FORM) & ": </label>"
					OutBuf(OUTPUT_FORM_CUSTOMMETA) = OutBuf(OUTPUT_FORM_CUSTOMMETA) & "<input type=""text"" id=""" & labelID & """ name=""" & metafields(fieldnum)(METAFIELD_NAME) & """ size=""20"" value=""" & meta_query(fieldnum) & """ class=""zoom_metaform_text"" />" & VbCrLf
				end if
			next
		...
		if (bSkipped = False) then
			...
			if (WordNotFound <> 1) then
				...
				for zoomit = 0 to dict_count
					...
					elseif (UseWildCards(sw) = 1) then
						Dim varWord
						for vi = 1 to dict(DICT_VARCOUNT, zoomit)
